...
function extendEvent (event, call, base, args, data) {
    event.flow = call;
    event.data = data;
    event.args = args;
    event.base = base;
...
module.exports = (resolved) => {
    return new Promise((resolve, reject) => {
        ...
        const next = (err, chunk, stream) => {
            ...
            event.output = extendEvent(stream || event.output, call, event.base, args, data);
        ...
        event.output = extendEvent(event.output, call, event.base, args, data);
        callHandler(event.output, handler, next, reject);
    });
};
