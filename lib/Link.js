...
module.exports = (scope, sequence, onEnd, onError, stream) => {
    ...
    // end event
    if (onEnd) {
        stream._o.on('end', () => scope.flow(onEnd).end(stream._chunk));
    }
...
function createHandler (scope, stream, handler) {
    ...
    if (handler.t === 'S') {
        if (!(handler_stream = handler.fn(scope, handler.s, handler_args, stream))) {
            return stream.emit('error', new Error('Link.stream: Empty return.'));
        }
    }
...
function transform (scope, args, objectMode, handler) {
    ...
    const stream = Transform({
        ...
        transform: (chunk, enc, next) => {
            try {
                handler.fn(scope, handler.s, args, chunk, next, stream, enc);
            } catch (err) {
                next(err);
            }
        }
