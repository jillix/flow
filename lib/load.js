...
module.exports = (scope, node, instance, event, event_id, args) => {

    // handle cached event
    let parsed_event = scope.cache.get('l:' + event_id);
    if (parsed_event) { 
        return process.nextTick(node.link.bind(node, args, parsed_event));
    }
    ...
    let triples = scope.read(instance, event, args);
