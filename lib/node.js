...
module.exports = function Node (event) {
    ...
    node._seq = function (output) {
        ...
        if (output) {
            ...
            if (isStream.isReadable(output) && !isStream.isDuplex(output)) {
                node._oow = true;
                node._o = output;
                return;
            }

            if (isStream.isWritable(output) && !isStream.isDuplex(output)) {
                (node._o || node).pipe(output);
                return;
            }

            if (
                (output._event instanceof Array && output._chunks instanceof Array) ||
