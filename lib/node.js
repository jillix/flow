...
module.exports = function (scope, key, args) {

    var node = new EventEmitter();
    node._i = new PassThrough({
        objectMode: args.objectMode !== undefined ? args.objectMode : true
    });
    node._i.on('error', (error) => {
...
function link (args, event) {
        ...
        switch (sequence[0]) {
            ...
            case 0:
                sequence[1].forEach(function (handler) {
                    node.join(Sequence(args, handler));
                });
                break;
            ...
            case 1:
                args._ = sequence[1][2] || {};
                node.join(sequence[1][1].call(sequence[1][0], args, node._o || node._i));
                break;

            // flow emit
            case 2:
                args._ = sequence[1][2] || {};
                // TODO get flow stream earlier? and just pipe it here?
                node.join(sequence[1][0].flow(sequence[1][1], args));
                break;
        }
    });
    ...
    var output = (node._o || node);

    // end event
    if (event.e) {
        node._ee = instance.inst.flow(event.e, args);
    ...
    // error events
    if (event.r) {
        node._re = node._re || instance.inst.flow(event.r, args);
        node.on('error', (error) => {
