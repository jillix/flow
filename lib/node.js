...
module.exports = function (scope, key) {
    ...
    node.on('end', function () {
        //console.log('Flow.node.end(event):', node.name, node.ready, node.ended);
        delete scope.streams[node.name];
    });
...
//console.log((output.name || 'Seq of ' + this.name), 'Duplex pipe to', (this._o ? '"output"' : '"input"'), 'and set as output.');
