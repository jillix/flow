...
exports.module = (loader, subject, predicate, object) => {
    ...
    // get cached module
    let cached_module = scope.cache.get('m:' + object);
    if (cached_module) {
        console.log('Module cached!');
        loader.emit('module', cached_module);
        //build(loader, scope.modules[composition.module]);
        return;
    }

    if (cached_module === null) {
        console.log('Module is loading!', object);
        return;
    }

    scope.cache.set('m:' + object, null);
    ...
    //  loader.scope.cache() = module;
    //  build(loader.scope, loader, composition, module);
    //});
...
exports.emit = (loader, subject, predicate, object) => {
    let instance = object;
    let event = object;
    loader.scope.flow(object);
};
