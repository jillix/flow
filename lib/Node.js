...
function createStream (node, args, handler) {
    ...
    switch (handler.type) {
        ...
        // flow emit
        case 2:
            stream = handler.method.flow(handler.instance, handler_args);
            break;
    }
...
function join (node, args, handler, handlers) {
    ...
    // set last stream as node output
    } else {
        node._o = A;
    }

    // pipe node to first stream in sequence
...
function link (args, event) {
    ...
    for(let handler in event.handlers) {
        join(node, args, handler, event.handlers);
    };
