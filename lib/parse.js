...
module.exports = function Parse (scope, emitter, key, instance, event, options, cb) {

    if (scope.get(scope.events, key, emitter, cb)) {
        Load(scope, scope.events[key], instance, options, function (instance) {
            process.nextTick(parse, scope, scope.events[key], instance, event, options);
        });
    }
};
