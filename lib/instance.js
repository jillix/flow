...
function _factory (adapter, name, session, callback) {
    ...
    // get composition
    adapter.mic(name, function (err, composition) {
        ...
        // pre load instances
        if (composition.load) {
            composition.load.forEach(adapter.load.bind(adapter));
        }

        // get CommonJS module
        adapter.mod(composition.module, function (err, module) {

            if (err) {
                return callbacks(name, err);
            }

            build(adapter, module, composition, callback, (name === '*'));
        });
    });
};
...
function build (adapter, module, config, callback, isEntrypoint) {

    // create new flow emitter
    var instance = adapter.factory(module);
